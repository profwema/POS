    ...
    $this->cndriver=$cndriver;
        if($cndriver=="mysql") {
            ...
            if(!$this->con) {
                ...
                if($this->myconn) {
                    $seldb = @mysqli_select_db($db_or_dsn_name,$this->myconn);
                    if($seldb) {
                        $this->con = true;
                        return true;
                    }
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
        ...
        if($cndriver=="odbc") {
            ...
            while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)) {
             ...
             @mysqli_query($adController->MySQL,"set names 'utf8'");
            $result = @mysqli_query($adController->MySQL,$this->sql); //query from db
...
//echo "$pchartfolder/class/pData.class.php";die;
    ...
    $result = @mysqli_query($adController->MySQL,$sql); //query from db
    $chartdata=array();
    $i=0;
//echo $sql."<br/><br/>";
    $seriesname=array();
...
/*       $rowheight="";
   ...
   $this->pdf->writeHTML($table);
    ...
    public function query($sql){
        ...
        return @mysqli_query($adController->MySQL,$sql);
    }
