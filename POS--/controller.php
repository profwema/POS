...
// تحقق CSRF لطلبات POST التي تنفذ وظائف معدلة للبيانات
...
class AdController
{
    ...
    {
        //    $this->MySQL =  mysqli_connect("localhost","root","",'wamtech_db') ; 
        //    if(!$this->MySQL)
        //    {
        //	//echo 'error';
        //	    $this->MySQL = mysqli_connect("178.238.228.170","root","R1HeK0D8a7R6CIqo",'wamtech_db') ; 
        //
        //    }
        //    $sSQL= 'SET NAMES utf8'; 
        //    mysqli_query($this->MySQL,$sSQL) ;	    


        include("db.conn.php");
        $this->MySQL        = $MySQL;
        $this->MySQLupdate  = $mysqli_Handle;
    }

    function __destruct()
    {
        mysqli_close($this->MySQL);
        mysqli_close($this->MySQLupdate);
    }
    ...
    {
        $sql = "SELECT id FROM storeuser WHERE email = ? LIMIT 1";
        list($ok, $stmt) = db_prepare_and_execute($this->MySQL, $sql, 's', [$email]);
        if (!$ok) return 0;
        $rows = db_fetch_all_assoc($stmt);
    ...
    function emailPresentEmp($email, $identifier, $id)
    {
        ...
        if ($identifier == 1)
            $sql = "SELECT id FROM employees WHERE email = ? LIMIT 1"; //
        else
            $sql = "SELECT id FROM employees WHERE email = ? AND id != ? LIMIT 1";

        if ($identifier == 1) {
            list($ok, $stmt) = db_prepare_and_execute($this->MySQL, $sql, 's', [$email]);
        } else {
            $id = (int)$id;
            list($ok, $stmt) = db_prepare_and_execute($this->MySQL, $sql, 'si', [$email, $id]);
        }
        if (!$ok) return 0;
        $rows = db_fetch_all_assoc($stmt);
        mysqli_stmt_close($stmt);
    ...
    function userPresent()
    {
        ...
        $sql = "SELECT id FROM employees WHERE user = ? LIMIT 1";
        list($ok, $stmt) = db_prepare_and_execute($this->MySQL, $sql, 's', [$username]);
        if ($ok) {
            $rows = db_fetch_all_assoc($stmt);
            mysqli_stmt_close($stmt);
        } else {
    ...
    function emailPresentSupp($email, $id)
    {
        ...
        $query         = "SELECT * FROM suppliers WHERE email = '$email' AND email !=''   AND id!='$id'";

        $res        = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
    ...
    function isExsist($table, $field, $value, $id, $chick)
    {
        ...
        if ($chick == 0)
            ...
            $query         = "SELECT * FROM $table WHERE $field = '$value'  AND id !='$id'";
        $res        = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
        $num        = mysqli_num_rows($res);
        return  $num;
    }
    ...
    {
        $query        = "SELECT * FROM main_settings WHERE storeid='1'";
    ...
    function getItemdata()
    {
        ...
        $resItem                = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
        $dataItem               = mysqli_fetch_assoc($resItem);

        $unitquery              = "SELECT * FROM units WHERE id = '" . $dataItem['unit'] . "'";
        $unitVal                = mysqli_query($this->MySQL, $unitquery);
    ...
    {
        $language     = $_SESSION['lang'];
        $queryCode  = "SELECT name_$language AS name FROM customers WHERE id = '$custId' ";
        $resCode    = mysqli_query($this->MySQL, $queryCode);
    ...
    function getStores()
    {
        ...
        $query         = "SELECT * FROM stores WHERE  branch ='$cid'  ORDER BY name_$language ASC";
        $res           = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
        while ($data = mysqli_fetch_assoc($res)) {
            $id   = $data['id'];
            $name = $data['name_' . $language];
    ...
    function search_items()
    {
        ...
        $storeid        = $_SESSION['storeid'];
        $term                   = $_REQUEST['term'];
        $query            = "SELECT * FROM items WHERE ( barcode LIKE '$term%' OR name_en LIKE '%$term%' OR name_ar LIKE '%$term%')  ";
        $resVal            = mysqli_query($this->MySQL, $query);
    ...
    function addBranch()
    {
        ...
        if (!$error) {
            ...
            $query    = "INSERT INTO  `branches` (
				...
				VALUES ('$name',   '$city',  '$state',  '$street',  '$street_ar',  '$country',  '$phone',  '$fax',  '$name_ar',  '$crnum',  '$crapp',  '$crexp','$munum', '$muapp',  '$muexp')";
    ...
    function delete_data()
    {
        ...
        echo mysqli_query($this->MySQL, $query);
    ...
    function delete_trans()
    {
        ...
        $query     = "UPDATE store_trans SET `state` ='0'
                         WHERE trans_number='$invoice' ";
        $exe = mysqli_query($this->MySQL, $query);
        $query     = "UPDATE store_trans_items SET`state` ='0'
                             WHERE trans_number='$invoice' ";
        $exe = mysqli_query($this->MySQL, $query);

        //                    $tData     = mysqli_fetch_assoc(mysqli_query($this->MySQL,"SELECT trans_number FROM store_trans WHERE  id='$idval' AND storeid='$storeid'"));
        //                    $transNo   = $tData['trans_number'];
        //                    $tData     = mysqli_fetch_assoc(mysqli_query($this->MySQL,"SELECT id FROM income WHERE purchase_invoice= '$transNo' AND storeid='$storeid'"));
        //                    $insertidIncome   = $tData['id'];
        //                    $tData     = mysqli_fetch_assoc(mysqli_query($this->MySQL,"SELECT id FROM outgo WHERE purchase_invoice= '$transNo' AND storeid='$storeid'"));
    ...
    {
        $storeid     = $_SESSION['storeid'];
        $query         = "SELECT GROUP_CONCAT(id) AS lst FROM branches   ORDER BY name_en ASC";
        $res           = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
        $data        = mysqli_fetch_assoc($res);
        $branch        = $data['lst'];
        ...
        $query        = "UPDATE items SET store_branch ='$branch' WHERE allbranches='1' ";
        mysqli_query($this->MySQL, $query);
    }
    ////// END BRANCHES //////
    ...
    function addCat()
    {
        ...
        if (count($branch) > 0) {
            $allBranches        = '0';
            $query     = "SELECT GROUP_CONCAT(id) AS lst FROM branches WHERE  id IN (" . implode(',', $branch) . ") ORDER BY name_en ASC";
        } else {
            $allBranches        = '1';
            $query     = "SELECT GROUP_CONCAT(id) AS lst FROM branches  ORDER BY name_en ASC";
        }
        ...
        if (!$error) {
			...
			`name_ar` ,
			`hash`,`allbranches`)
			VALUES ( '$enabled',  '$name',  '$branch','$show_to_cashier','$name_ar','$hash','$allBranches')";
            $exe = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
    ...
    function updateCat()
    {
        ...
        if (!$error) {
			...
			`name_ar` ='$name_ar' ,allbranches='$allBranches' WHERE ` id='$catid'";
            $exe = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
            if ($exe) {
    ...
    function BarcodeExist($barcode, $id)
    {
        ...
        $storeid        = $_SESSION['storeid'];
        $query = "SELECT id FROM items WHERE barcode='$barcode' AND barcode!='' AND id !='$id' ";
        $res    = mysqli_query($this->MySQL, $query);
        $num    = mysqli_num_rows($res);
        if ($num) {
            $data    = mysqli_fetch_assoc($res);
    ...
    function addMultiItem()
    {
        ...
        for ($i = 0; $i < count($barcode); $i++) {
            if (!$this->BarcodeExist($barcode[$i], 0))
                exit();
        }
    ...
    function updateMultiItem()
    {
                ...
                if ($idArray[$i] != '') {
                    ...
                    $query         = "UPDATE `items` SET
							...
							item_type       ='$Itype',
                                                        measurement     ='$Qtype',   
							services        ='$services'
						 WHERE item_thread='$hashofItem'  AND id='$idV'";
                    $exe = mysqli_query($this->MySQL, $query);
                    if ($exe) {
        ...
        if (count($arrayOfUpdates)) {
            ...
            $query            = "DELETE FROM items WHERE item_thread='$hashofItem'  AND id NOT IN ($arrayOfUpdates)";
            mysqli_query($this->MySQL, $query);
        }
        if ($insertedItem > 0)
            echo $insertedItem;
    ...
    function addEmp()
    {
        ...
        $passport_expiry    = ($passport_expiry     != "") ? $this->mysqlCompatible($passport_expiry) : "NULL";
        $iqama_expiry       = ($iqama_expiry        != "") ? $this->mysqlCompatible($iqama_expiry) : "NULL";
        $insurance_expiry   = ($insurance_expiry    != "") ? $this->mysqlCompatible($insurance_expiry) : "NULL";
        $medical_expiry     = ($medical_expiry      != "") ? $this->mysqlCompatible($medical_expiry) : "NULL";
        $license_expiry     = ($license_expiry      != "") ? $this->mysqlCompatible($license_expiry) : "NULL";
    ...
    function updateEmp()
    {
        ...
        if (!$error) {
            ...
            if ($_POST['pass']) {
                ...
                $pwdVal            = $_POST['pass'];
                $password                   = md5($pwdVal); //$aes->encrypt();   
                $query = "UPDATE `employees` SET `password`='$password'  WHERE  id='$itemId'";
                $exe = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
            }
            $query = "UPDATE  `employees`  SET 
                        ...
                        visa = '$visa',
                        mada = '$mada',
                        `name_ar` ='$name_ar'  WHERE  id='$itemId'";
            $exe = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
            if ($exe) {
    ...
    function getBranchCategories()
    {
        ...
        if ($bid != "") {
            $output = array();
            if ($bid != "" && $bid != "null")
                $query     = "SELECT * FROM categories WHERE  FIND_IN_SET('$bid',store_branch) OR store_branch IN ($bid) ORDER BY name_en ASC";
            else
    ...
    function addExpenditure()
    {
        ...
        if (!$error) {
            ...
            $query = "INSERT INTO `expenditure` (`store_branch`, `dated`, `desc_ar`, `cost`,  `desc_en`) "
                . "VALUES ('$branch', '$dated', '$desc_ar', '$cost',  '$desc')";
            $exe = mysqli_query($this->MySQL, $query);
            if ($exe)
    ...
    function updateExpenditure()
    {
        ...
        if (!$error) {
            $dated    = $this->mysqlCompatible($dated);
            $query = "UPDATE `expenditure` SET `store_branch`='$branch', `dated`='$dated', `desc_ar`='$desc_ar', `cost`='$cost', `desc_en`='$desc' WHERE id='$catid'  ";
            $exe = mysqli_query($this->MySQL, $query);
            if ($exe)
    ...
    function discountAdd()
    {
        ...
        if (!$error) {
            $query         = "SELECT * FROM discount WHERE branchid='$branch' ";
            $res        = mysqli_query($this->MySQL, $query);
            $dataDiscount    = mysqli_fetch_assoc($res);
            $num        = mysqli_num_rows($res);
            if (!$num)
                $query = "INSERT INTO discount(amount,branchid,minamount) VALUES('$discount','$branch','$minamount')";
            else
                $query = "UPDATE discount SET amount='$discount',minamount='$minamount' WHERE branchid='$branch' ";
    ...
    function updateDelivery()
    {
        ...
        $num        = mysqli_num_rows($res);
        if (!$num)
            $query = "INSERT INTO delivery(amount) VALUES('$discount')";
        else
            $query = "UPDATE delivery SET amount='$discount' ";

        echo $exe   = mysqli_query($this->MySQL, $query);
    }
    ...
    function updatepalDate()
    {
        ...
        $query         = "SELECT * FROM open_palance_date ";
        $res        = mysqli_query($this->MySQL, $query);
        $dataDiscount    = mysqli_fetch_assoc($res);
        $num        = mysqli_num_rows($res);
        if (!$num)
            $query = "INSERT INTO open_palance_date(date) VALUES('$date')";
        else
            $query = "UPDATE open_palance_date SET date='$date' ";

        if ($exe   = mysqli_query($this->MySQL, $query))
            $state = 1;
        $query = "UPDATE open_palance SET date='$date'";
        if ($exe   = mysqli_query($this->MySQL, $query))
            $state = 1;
        echo $state;
    }
    ...
    function vatAdd()
    {
        ...
        if (!$num)
            ...
            $query = "UPDATE vat_default SET percentage='$discount' ";
        echo $exe   = mysqli_query($this->MySQL, $query);
    }
    ...
    function tableUpdate()
    {
        ...
        $error = $obj->validate();
        if (!$error) {
            $query     = "SELECT id FROM tables_count ";
            $res    = mysqli_query($this->MySQL, $query);
            $num    = mysqli_num_rows($res);
            if ($num)
                $query = "UPDATE `tables_count` SET `table_c`='$table', `branch`='$branch' ";
            else
                $query = "INSERT INTO `tables_count`(`table_c`) VALUES('$table') ";
    ...
    {
        $storeid = $_SESSION['storeid'];
        $query     = "SELECT COUNT(*) AS allc FROM employees ";
        $res    = mysqli_query($this->MySQL, $query);
        $data    = mysqli_fetch_assoc($res);
    ...
    {
        $storeid = $_SESSION['storeid'];
        $query     = "SELECT SUM(cost) AS allc FROM sales ";
        $res    = mysqli_query($this->MySQL, $query);
        $data    = mysqli_fetch_assoc($res);
    ...
    {
        $storeid = $_SESSION['storeid'];
        $query     = "SELECT COUNT(*) AS allc FROM items ";
        $res    = mysqli_query($this->MySQL, $query);
        $data    = mysqli_fetch_assoc($res);
        echo     intval($data['allc']);
    }
    ...
    {
        $storeid = $_SESSION['storeid'];
        $query     = "SELECT SUM(cost) AS allc FROM expenditure ";
        $res    = mysqli_query($this->MySQL, $query);
        $data    = mysqli_fetch_assoc($res);
        echo     round(floatval($data['allc']), 2);
    }
    ...
    function updateService()
    {
        ...
        if (!$error) {
            ...
            $query = "UPDATE  `services` SET
                            `name_en` ='$name',
                            `name_ar` ='$name_ar'  WHERE  id='$service'";
            $exe = mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
            if ($exe) {
    ...
    function shiftsUpdate()
    {
        ...
        for ($i = 0; $i < count($idv); $i++) {
            ...
            if ($nameEn != '' || $nameAr != '') {
                if (intval($idValue) > 0)
                    $query  = "UPDATE shifts SET starts='$start', ends='$ends',name_en='$nameEn',name_ar='$nameAr' WHERE  id='$idValue' ";
                else
                    $query  = "INSERT INTO shifts(name_en,name_ar,starts,ends) VALUES('$nameEn','$nameAr','$start','$ends')";
    ...
    function storesUpdate()
    {
        ...
        for ($i = 0; $i < count($idv); $i++) {
            ...
            if ($nameEn != '' || $nameAr != '') {
                ...
                if (intval($idValue) > 0) {
                    $query  = "UPDATE stores SET name_en='$nameEn',name_ar='$nameAr',branch='$brch' WHERE  id='$idValue' ";
                    mysqli_query($this->MySQL, $query);
                    ...
                    $query  = "INSERT INTO stores(name_en,name_ar, branch ) VALUES('$nameEn','$nameAr','$brch')";
                    mysqli_query($this->MySQL, $query);
                    $id      = mysqli_insert_id();
    ...
    function jobTitleUpdate()
    {
        ...
        for ($i = 0; $i < count($idv); $i++) {
            ...
            if ($nameEn != '' || $nameAr != '') {
                if (intval($idValue) > 0)
                    $query  = "UPDATE jobtitle SET name_en='$nameEn',name_ar='$nameAr' WHERE  id='$idValue' ";
                else
                    $query  = "INSERT INTO jobtitle(name_en,name_ar) VALUES('$nameEn','$nameAr')";
    ...
    function getBalanceItem($item, $store, $old)
    {
        ...
        $resItem  =    mysqli_query($this->MySQL, $queryItems) or die(mysqli_error($this->MySQL));        //echo $queryAll."<br>";hhh
    ...
    function setSales($invoice, $time, $outgo_No, $journalNo)
    {
        ...
        $visa =   ($_POST['visa']) ? $_POST['visa'] : 0;
        $mada =   ($_POST['mada']) ? $_POST['mada'] : 0;
        $left =   $_POST['left'];
        ...
        $pExpiry = date('Y-m-d H:i:s', strtotime($pExpiry));
        ...
        $queryOutgo    = "INSERT INTO outgo (outgo_No     ,branch    ,invoice_type, Beneficiary, invoice_No, date,  user,    dis_added        , all_total ,storeid) VALUES"
            . "('$outgo_No','$branch' ,'SALES'  , '$customer', '$invoice', '$pExpiry','$user','$final_discount','$totalAll','$storeid')";
        $exe            = mysqli_query($this->MySQL, $queryOutgo) or die(mysqli_error($this->MySQL));;
        ...
        for ($i = 0; $i < count($itemsArray); $i++) {
            ...
            if ($store != '' && $itemId != '' && $qty != '' && $price != '') {
                ...
                mysqli_query($this->MySQL, $query) or die(mysqli_error($this->MySQL));
    ...
    function addSale()
    {
        ...
        if ($chick == 1) {
            ...
            $time        =   date("h:i:s");
            $set = $this->setSales($invoice, $time, $outgo_No, $journalNo);
            $_SESSION['result'] = 'SUCCESS';
            if (isset($_POST['cashier'])) {

                require_once("escpos/index.php");
    ...
    function setPSaleRet($invoice, $time, $income_No, $journalNo)
    {
        ...
        $exe         = mysqli_query($this->MySQL, $queryOutgo);
    ...
    function setPurchase($invoice, $time, $income_No, $journalNo)
    {
        ...
        $exe         = mysqli_query($this->MySQL, $queryIncome);
    ...
    function editStoreTrans()
    {
        ...
        $incomeData     = mysqli_fetch_assoc(mysqli_query($this->MySQL, "SELECT income_No FROM income WHERE invoice_No='$invoice' AND storeid='$storeid'"));
        ...
        $outgoData      = mysqli_fetch_assoc(mysqli_query($this->MySQL, "SELECT outgo_No FROM outgo WHERE invoice_No='$invoice' AND storeid='$storeid'"));
        ...
        for ($i = 0; $i < count($itemsArray); $i++) {
            ...
            if ($itemId != '' && $qty != '') {
                ...
                $res23      = mysqli_query($this->MySQL, $query);
                ...
                mysqli_query($this->MySQL, $query);
                mysqli_query($this->MySQL, $queryIncome);
                mysqli_query($this->MySQL, $queryOutgo);
            }
        }
        ...
        if (mysqli_num_rows($query) > 0) {
            while ($row = mysqli_fetch_assoc($query)) {
                $sql = "UPDATE accounts SET
                            ...
                            budget='$flbs'
                            WHERE id ='$row[id]'";
                $exe = mysqli_query($this->MySQL, $sql) or die(mysqli_error($this->MySQL));
    ...
    {
        $result = mysqli_query($this->MySQL, "SELECT count(*) as total FROM `accounts` WHERE  `parent` = $id");
    ...
    function getNewCode()
    {
        ...
        $resCode    = mysqli_query($this->MySQL, $queryCode) or die(mysqli_error($this->MySQL));
    ...
    function getAcctCode($table, $parent, $refId)
    {
        ...
        if (mysqli_num_rows($resCode) > 0) {
            ...
            $resFile    = mysqli_query($this->MySQL, $queryFile);
    ...
    function updateAcount($name_en, $name_ar, $pairent, $fileId)
    {
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            $id = $row['id'];
            mysqli_query($this->MySQL, "UPDATE accounts SET
                name_en='$name_en',
                name_ar='$name_ar'
                WHERE id ='$id'");
        } else {
    ...
    {
        $costCenter = ($costCenter) ? $costCenter : 0;
    ...
    {
        //$mysqli_Handle = updateConnect();
        $current = $_POST['current'];


        $sql_check = mysqli_query($this->MySQLupdate, "SELECT * FROM updates WHERE id > $current") or die(mysqli_error($this->MySQLupdate));
    ...
    function dbUpdate()
    {
        ...
        $sql = mysqli_query($this->MySQLupdate, "SELECT * FROM updates WHERE id > '$current'") or
            die(mysqli_error($this->MySQLupdate));
        $num = mysqli_num_rows($this->MySQLupdate);
        ...
        while ($row =  mysqli_fetch_array($sql)) {
            ...
            if (copy($file, $targetFile)) {
                ...
                for ($i = 1; $i < count($pieces); $i++) {
                    //echo $pieces[$i].'<br>';
                    $exe = mysqli_query($this->MySQL, $pieces[$i]);
                    if (!$exe) {
                        $done = 0;
                        echo mysqli_error($this->MySQL) . " ---- Error <br>";
                    }
                }
                unlink($targetFile);
            } else {
