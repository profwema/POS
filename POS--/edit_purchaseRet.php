...
$invoice        = $_REQUEST['sd'];
$query          = "SELECT * FROM purchase_ret WHERE invoicenumber='$invoice'";
$resP           = mysqli_query($adController->MySQL,$query);
$dataPur        = mysqli_fetch_assoc($resP);
$purchInvoice   = $dataPur['purchInvoice'];

$query		= "SELECT * FROM purchase_ret_items WHERE storeid='$storeid' AND invoice_No='$invoice'";
			...
			<div>
				<div class="box span12">
					<div class="box-header" data-original-title>
                                            <h2><i class="halflings-icon edit"></i><span class="break"></span><a href="purchaseRet.php"><?=RETURNED_PURCHASES?></a> >> <?=EDIT?></h2>
					</div>  
					<div class="box-content">
                                            <form class="form-horizontal" id="purchaseRet-form">
                                                <input type="hidden" value="editPurchaseRet" name="f">                                            <fieldset style="border-bottom:  #e7e7e7 solid medium">
                                                <div class="control-group">
                                                    <div class="salesOptions">
                                                      <label class="control-label" for="typeahead"><?=PURCHASE_INVOICE?> : </label>
                                                      <div class="controls">
                                                           <?php
						  ...
						  <input type="hidden" value="editPurchaseRet" name="f">
						  ...
						  <fieldset>   
							...
							<div class="control-group">
							  ...
							  <div class="controls">
                                                              <input type="hidden" value="<?=$dataPur[supplier]?>" name="supplier">
                                                                <?php
                                                                $query		  = "SELECT * FROM suppliers  WHERE id='$dataPur[supplier]' AND storeid='$storeid'";
                                        ...
                                        <div class="salesOptions">
                                            ...
                                            <div class="controls">
                                                ...
                                                <input type="radio" name="payType" value="1" <?=$dataPur['paymentType']=="1" ? "checked" : "" ?>><?=CASH?>
                                                 ...
                                                 <label>
                                                <input type="radio" name="payType" value="4" <?=$dataPur['paymentType']=="4" ? "checked" : "" ?>><?=LATER?>
                                                 </label>
                                            </div>
                                        </div>                                                             
							</div>                                                      
...
<!--                                                <TH ><?=QTY_AFTER?></TH>-->
                    ...
                    //										
                                    ...
                                    $query   = "SELECT items.*,purchase_items.store, purchase_items.cost, purchase_items.quantity, purchase_items.tax, purchase_items.discount AS sdis FROM items, purchase_items WHERE items.storeid='$storeid' AND items.id = purchase_items.itemid  AND purchase_items.invoice_No='$purchInvoice'";
