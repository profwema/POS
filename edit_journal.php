...
$no             = $_REQUEST['no'];
$query          = "SELECT * FROM journal WHERE journalNo='$no'";
$resP           = mysqli_query($adController->MySQL,$query);
$dataPur        = mysqli_fetch_assoc($resP);

$query		= "SELECT * FROM journal_items WHERE journalNo='$no'";
			...
			<div>
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon edit"></i><span class="break"></span><?=JOURNAL?> <?=EDIT?></h2>
					</div>
                                    <div class="box-content" id="printed">
						<form class="form-horizontal" id="journal-form">
                                                    
                                                    <input type="hidden" value="editJournal" name="f">
						  <fieldset>
							  ...
							  <div class="controls">
								...
								<div class="controls">
								  <input type="text" class="datepicker-nobar" name="date"  
                                                                         value='<?=date("d/m/Y",strtotime($dataPur['date']))?>'>
								</div>                                                           
							</div>                                                             
								...
								<div class="controls">
								  <select name="cost" data-rel="chosen">
                                        ...
                                        <div class="controls">
                                            <textarea class="span6 typeahead"  style="width: 90%" name="disc" id="disc" rows="5"  required='required'><?=$dataPur['disc']?></textarea>                                            
                                        </div>                                  
                                    </div>                                                              
                                                        </div>
                                    <div class="control-group" >
                                                ...
                                                <tr <?=$rowStyle?> id='row-v-<?=$i?>'>
                                                  ...
                                                  <select name="account[]" id='account-<?=$i?>'data-rel="chosen" style="width:90%" >
                                                      <option value=""> sdas</option>                                      
                                                        <?php
                                                        $language 	= $_SESSION['lang'];
                                                        $query = "SELECT * FROM  accounts WHERE hasChild ='0' ORDER BY name_$language ASC";
                                                        ...
                                                        while($data = mysqli_fetch_assoc($res))
                                                        {
                                                            ...
                                                            $parent= $dataP['parent'];
                                                            $defult='';
                                                                    if ($itemsListArray[$i]['accCode']== $code )
                                                                    {
                                                                        $defult=" selected ";
                                                                    }                                                                                                                                        
                                                            echo "<option value='$data[code]' $defult>$parent -> $name  </option>";
                                                        }
                                                    ...
                                                    <td style="width:10%">
                                                            <input type="text" id='debit-<?=$i?>' class="debit span12 typeahead" maxlength="100"  name='debit[]' value="<?=$itemsListArray[$i]['debit']?>">
                                                    </td>
                                                    <td style="width:10%">
                                                            <input type="text" id='credit-<?=$i?>' class="credit span12 typeahead" maxlength="100"  name='credit[]' value="<?=$itemsListArray[$i]['credit']?>">
                                                    </td>
                                                    <td style="width:30%">
                                                            <input type="text" id='disc-<?=$i?>' class="span12 typeahead" maxlength="100"  name='discItem[]'value="<?=$itemsListArray[$i]['disc']?>">
                                                    </td>
                                                    <td style="width:15%">
                                                            <input type="text" id='ref-<?=$i?>' class="span12 typeahead" maxlength="100"  name='ref[]'value="<?=$itemsListArray[$i]['reference']?>">
                                                    </td>
                                                    <td style="width:10%">
